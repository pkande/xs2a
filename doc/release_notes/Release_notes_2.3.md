...
## Bugfix: links building while getting account list
Implemented the new mechanism for links generation in the `getAccoutList` request (GET `/v1/accounts/{withBalance}`).
...
link is given. Boolean path parameter `withBalance` influences only the response for this request and has nothing common
with the links.
